...
var config_dir = require("./config").dir;
...
switch (os.platform()) {
    ...
    service = {
      install: function(done){
        fs.readFile(launchDTpl,{ encoding:"utf8" }, function(err,data){
          if (err) throw err;
          data = data.replace("{{ NODE_BIN }}",process.execPath).replace("{{ SERVICE_FILE }}",service_file).replace("{{ CONFIG_DIR }}",config_dir);
  ...
  case "linux":
    ...
    var dnsmasqTpl = path.join(path.dirname(module.filename),"../etc/linux/dnsmasq.tpl");
    var dnsmasq = "/etc/dnsmasq.d/dev";
    service = {
      install: function(done){
        fs.readFile(upstartTpl,{ encoding:"utf8" },function(err,data){
          ...
          fs.writeFile(upstart,data,{ encoding:"utf8" },function(err){
            if (err) throw err;
            exec("mkdir -p "+path.dirname(dnsmasq), function(err) {
              if (err) throw err;
              fs.readFile(dnsmasqTpl, { encoding:"utf8" }, function(err,data){
                if (err) throw err;
                fs.writeFile(dnsmasq, data, {encoding: "utf8"}, function(err) {
                  if (err) throw err;
                  if (done) done();
                });
              });
      ...
      remove: function(done){
        fs.unlink(upstart,function(err){
          if (err) throw err;
          fs.unlink(dnsmasq, function(err) {
            if (err) throw err;
            if (done) done();
          });
        });
